import { collection, deleteDoc, doc, getDoc, getDocs, increment, orderBy, query, serverTimestamp, setDoc, updateDoc, where } from "firebase/firestore";
...
export const uploadPDF = async (file, organizationId, metadata, onProgress) => {
  try {
    ...
    formData.append('folder', `uploads/${organizationId}`); // Organize by organization
    formData.append('resource_type', 'raw'); // For PDFs, use 'raw' resource type
    formData.append('public_id', `${uploadId}_${file.name.replace('.pdf', '')}`);
...
export const searchUploads = async (searchText, organizationId = null) => {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const tagMatch = data.tags?.some(tag => tag.toLowerCase().includes(searchLower));
...
export const getUploadStats = async (organizationId) => {
  try {
    ...
    const stats = {
      ...
      totalDownloads: uploads.reduce((sum, upload) => sum + (upload.downloadCount || 0), 0),
      byCategory: {},
      byType: {}
    };
